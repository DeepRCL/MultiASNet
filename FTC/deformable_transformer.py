...
from FTC.util.misc import inverse_sigmoid, NestedTensor
...
class DeformableTransformerEncoder(nn.Module):
    ...
    def forward(self, src, temporal_lens, level_start_index, valid_ratios, pos=None, padding_mask=None, prompt=None):
        '''
        src: shape=(bs, t, c)
        ...
        for _, layer in enumerate(self.layers):
            if prompt is not None:
                ...
                if s > 32:
                    output = output[:, -32:, :]
                output = torch.cat((prompt, output), dim=1)
...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, pos_embeds,masks ,query_embed=None, prompt=None):
        '''
        Params:
        ...
        # an extra one for the prompt
        #temporal_lens[0] = temporal_lens[0] + 2
        ...
        memory = self.encoder(src_flatten, temporal_lens, level_start_index, valid_ratios, 
            lvl_pos_embed_flatten, prompt=prompt)  # shape=(bs, t, c)
