...
class Network(object):
    ...
    def _restore(self, pt_file):
        ...
        # Loading model.
        self.model.load_state_dict(load_res["model"], strict=True)
        # Loading optimizer.
    ...
    def test(self, loader_te, mode="test"):
        ...
        for data in tqdm(loader_te):
            ...
            if self.config['cotrastive_method'] == 'CE' or self.config['cotrastive_method'] == "Linear":
                ...
                if self.config['model'] == "FTC_TAD":
                    pred_AS, _, _, _, _, _, _ = self.model(cine, tab_data, target_B, target_age, split='Test') # Bx3xTxHxW
                else:
    ...
    def test_comprehensive(self, loader, mode="test",record_embeddings=False):
        ...
        ce_losses = []
        cine_videos = []  # Store cine tensors for high CE samples
        
        for cine, tab_info, target_AS, target_B, target_age, data_info, cine_orig in tqdm(loader):
        ...
        # save the dataframe
        test_results_file = os.path.join(self.log_dir, mode+"_revision.csv")
    ...
    # Function to save cine as a video
    def save_video(self, video_array, filename, pred_AS):
        """Save a (C, T, H, W) video as an MP4 file."""
        fps=10
        video_array = self.norm_to_bin(video_array)
        C, T, H, W = video_array.shape  # (3, 32, 224, 224)
        
        # Convert from (C, T, H, W) → (T, H, W, C) for OpenCV
        video_array = video_array.transpose(1, 2, 3, 0)  # (T, H, W, C)
        
        # Normalize to 0-255 (assuming input is in range 0-1)
        video_array = (video_array * 255).astype(np.uint8)

        # Convert frames to PIL images
        frames = [Image.fromarray(frame) for frame in video_array]
        ...
        frames[0].save(
            ...
            duration=int(1000 / fps),  # Convert FPS to duration (ms per frame)
            loop=0  # Infinite loop
        )

    def norm_to_bin(self, norm_tensor):
        """Unnormalize a tensor that was normalized using (x - m) / std."""
        m = 0.099
        std = 0.171
        return norm_tensor * std + m  # Reverse normalization
